...
def _banner(root: Path) -> list[str]:
    """Identity of the generator, stable across machines.

    A home path in the committed map is the same class of bug as a timestamp:
    CI regenerates against a different checkout and ``--check`` fails forever.
    """
    this_repo = root.resolve() in {PROFILE.resolve(), _canonical_profile().resolve()}
    if this_repo:
        here = "scripts/gen_codemap.py"
        regen = "`make map`"
        check = "`make map-check`"
    else:
        profile = _canonical_profile()
        try:
            prefix = "~/" + str(profile.relative_to(Path.home()))
        except ValueError:
            prefix = str(profile)
        here = f"{prefix}/scripts/gen_codemap.py"
        regen = f"`python3 {here}`"
        check = f"`python3 {here} --check`"
    return [
        f"<!-- GENERATED by {here} — DO NOT EDIT.",
...
def self_test() -> int:
    ...
    if first != second:
        ...
        return 1
    this_banner = first[str(SKELETON)].splitlines()[0]
    if "GENERATED by scripts/gen_codemap.py" not in this_banner:
        print(
            "this-repo map banner must be machine-independent "
            f"(got {this_banner!r})",
            file=sys.stderr,
        )
        return 1
